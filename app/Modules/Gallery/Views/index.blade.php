            ...
            <div class="row">
                ...
                <div class="col-sm-7">
                    <div class="btn-toolbar float-right" role="toolbar" aria-label="@lang('labels.general.toolbar_btn_groups')">
                        <a href="{!! route('gallery.create') !!}" class="btn btn-success ml-1" data-toggle="tooltip" title="@lang('labels.general.create_new')"><i class="fas fa-plus-circle"></i></a>
                    </div><!--btn-toolbar-->
                </div><!--col-->
            </div><!--row-->
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @foreach($photos as $photo)
                                <tr>
                                    <td>{{ $photo->id }}</td>
                                    <td>{{ isset($photo->title) ? $photo->title : 'N/A' }}</td>
                                    <td> {!! isset($photo->description) ? $photo->description : 'N/A' !!}
                                    </td>
                                    <td>@if($photo->image)
                                            <img src="/uploads/gallery/{{ $photo->image }}" height="50" width="100">
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    <td>
                                        <form  action="{{ route('gallery.destroy',$photo->id) }}" method="post">
                                            @method('DELETE')
                                            {!! csrf_field() !!}
{{--                                            <a href="{{ route('event.edit',$photo->id) }}" class="btn btn-sm btn-info"title="Edit"><i class="fa fa-edit"></i></a>--}}
