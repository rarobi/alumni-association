                        ...
                        <table class="table">
                            ...
                            @foreach($writers as $writer)
                                <tr>
                                    <td>{{ $writer->id }}</td>
                                    <td>{{ $writer->name }}</td>
                                    <td>{{ str_limit( $writer->writer_details, 25)  }}</td>
                                    <td>{!! $writer->created_at !!}</td>
                                    <td>
                                        <form  action="{{ route('settings.book.writers.destroy',$writer->id) }}" method="post">
                                            @method('DELETE')
                                            {!! csrf_field() !!}
                                            <a href="{{ route('settings.book.writers.edit',$writer->id) }}" class="btn btn-sm btn-info"title="Edit"><i class="fa fa-edit"></i></a>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title w-100 font-weight-bold">Add book writer</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    {{ html()->form('POST', route('settings.book.writers.store'))->class('form-horizontal')->open() }}
